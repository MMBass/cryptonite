...
const TEMPLATES = {
 COIN_TEMPLATE: function(symbol,name,Id){
   return `<div class="card-parent col-lg-4 col-xs-12 col-md-6 col-sm-6 mt-2" data-symbol-parent="${symbol}">
    <div class="card" data-name="${name}" data-id="${Id}" data-seconds="120">
     <h5 class="card-title p-2">${symbol}</h5>
     <div class="card-body">
       <p class="card-text p-2">${name}</p>
       <a class="btn btn-primary m-2" onclick="openMoreInfo(this)"data-click='no'>More info</a>
   ...
   CHART_EMPTY:`
      ...
      <div class="card-body">
        <h1 class="card-title">Nothing to show</h1>
...
const NAV_BAR = {
   navBtns:function(target){
     ...
     $('.navbar a').css('border-bottom','3px solid #0e1e30');
     $(target).css('border-bottom','3px solid #00d548');
     clearInterval(liveReport.chartInterval)
     $("#chartContainer_div").html('');
     liveReport.secondsCount = 0;
     $(`.card-parent`).attr('class','card-parent col-lg-4 col-xs-12 col-md-6 col-sm-6 ');
   ...
   $('#search_btn').on('click',function(){
    ...
    if($itsClass != undefined || $val == "selected-items"){
     ...
     for(i = 0; i < allCardsCoins.length; i++){
         allCardsCoins[i].classList = "card-parent col-lg-4 col-xs-12 col-md-6 col-sm-6 d-none";
     }
     if($val == "selected-items"){
       for(i in liveReport.itemsInLive){
         $(`.card-parent[data-symbol-parent='${(liveReport.itemsInLive[i]).toLowerCase()}']`).attr('class','card-parent col-lg-4 col-xs-12 col-md-6 col-sm-6');
       }
     }else{
         $(`.card-parent[data-symbol-parent='${$val}']`).attr('class','card-parent col-lg-4 col-xs-12 col-md-6 col-sm-6');
     }
    }
